...
import { ChapmanCGLogEntry } from '../types';

interface AddChapmanCGLogModalProps {
  entry: ChapmanCGLogEntry | null;
  onClose: () => void;
  onSave: (entry: Omit<ChapmanCGLogEntry, 'id' | 'addedBy' | 'timestamp'>) => void;
  existingEntries: ChapmanCGLogEntry[];
}

export function AddChapmanCGLogModal({ entry, onClose, onSave, existingEntries }: AddChapmanCGLogModalProps) {
  const isEditing = !!entry;
  
  const [formData, setFormData] = useState({
    clientName: '',
    subjectIssue: '',
    category: 'others' as ChapmanCGLogEntry['category'],
    dateStarted: '',
    timeStarted: '',
    dateFinished: '',
    timeFinished: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const creditConsumed = timeConsumedMinutes / 60;
    const totalCreditConsumed = existingEntries.reduce((sum, entry) => sum + entry.creditConsumed, 0) + creditConsumed;
  ...
  const previewCreditConsumed = previewTimeConsumed / 60;
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-xl font-semibold">
                  {isEditing ? 'Edit Log Entry' : 'Add New Log Entry'}
                </h2>
                <p className="text-blue-100 text-sm">Complete the form below to {isEditing ? 'update' : 'create'} a ChapmanCG log entry</p>
              </div>
            </div>
            <button
              onClick={onClose}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <span className="ml-2 font-medium text-blue-900">
                          {Math.floor(previewTimeConsumed / 60)}h {previewTimeConsumed % 60}m
                        </span>
                      </div>
                      <div>
                        <span className="text-blue-700">Credit Consumed:</span>
                        <span className="ml-2 font-medium text-blue-900">{previewCreditConsumed.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                )}
