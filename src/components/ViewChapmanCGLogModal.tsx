...
interface ViewChapmanCGLogModalProps {
  entry: ChapmanCGLogEntry;
  onClose: () => void;
}

export function ViewChapmanCGLogModal({ entry, onClose }: ViewChapmanCGLogModalProps) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900">{entry.idCode}</h2>
              <p className="text-xs text-gray-600">{entry.clientName}</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="bg-white p-3 rounded-lg border">
                    ...
                    <p className="text-lg font-semibold text-blue-600">{entry.timeConsumedMinutes} min</p>
                  ...
                  <div className="bg-white p-3 rounded-lg border">
                    ...
                    <p className="text-lg font-semibold text-purple-600">{entry.totalTimeChargeMinutes} min</p>
                    <p className="text-sm text-gray-500">{formatTime(entry.totalTimeChargeMinutes)}</p>
                  </div>
                  <div className="bg-white p-3 rounded-lg border">
                    <label className="block text-sm font-medium text-gray-600">Credit Consumed</label>
                    <p className="text-lg font-semibold text-green-600">{entry.creditConsumed.toFixed(2)}</p>
                    <p className="text-sm text-gray-500">credits</p>
                  </div>
                  <div className="bg-white p-3 rounded-lg border">
                    <label className="block text-sm font-medium text-gray-600">Total Credit Consumed</label>
                    <p className="text-lg font-semibold text-orange-600">{entry.totalCreditConsumed.toFixed(2)}</p>
                    <p className="text-sm text-gray-500">credits</p>
                  </div>
                </div>
              </div>
